...
export default function ChannelList({
  ...
  return (
    <div
      className={`${open ? 'w-60' : 'w-16'} duration-300 h-full flex flex-col bg-med-grey p-4 relative`}
    >
      <img
            ...
            return (
              ...
              >
                <img
                  src={iconPath}
                  alt="icon"
                  className={`cursor-pointer duration-500 w-10 h-10 mr-5`}
                />
                <span className={`${open ? 'block' : 'hidden'}`}>{channel.name}</span>
              </li>
            )
          })}
        </ul>
      ...
      <div
        className={`${open ? 'opacity-100 max-h-screen visible' : 'opacity-0 max-h-0 invisible'} transition-opacity duration-300 ease-in-out overflow-hidden`}
      >
        <ConnectionControls
